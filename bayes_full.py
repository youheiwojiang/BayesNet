...
#---------------------------------------------------------------------------
def sample(size):
    trainsample = random.sample(list(traindata),size)
    return trainsample

def predictions_tan(size):
    trainsample = sample(size)
    tree = generatetree(trainsample)
    num = 0
    for user in testdata:
        list = calprob_tan(tree,user,trainsample)
        if trainmeta[attr[col-1]][1][list.index(max(list))] == user[col-1].decode("utf-8"):
            num+= 1
    return num/len(testdata)

def predictions_nb(size):
    e = [[] for x in range(col-1)]
    trainsample = sample(size)
    num = 0
    for user in testdata:
        list = calprob_tan(e,user,trainsample)
        if trainmeta[attr[col-1]][1][list.index(max(list))] == user[col-1].decode("utf-8"):
            num+= 1
    return num/len(testdata)
...
def plotfigure():
    ...
    accurate_nb = [0]
    accurate_tan = [0]
    for size in array[1:5]:
        right = 0
        for i in range(4):
           right +=  predictions_tan(size)
        accurate_tan.append(right/4)
        right = 0
        for i in range(4):
           right +=  predictions_nb(size)
        accurate_nb.append(right/4)
    pl.plot(array,accurate_nb,'r-*',label = 'Naive Bayes')
    pl.plot(array,accurate_tan,'b--+',label = 'Tan')
    pl.title("Learning Curve of Tan and Naive Bayes ")
